...
//        setEdgeDefaultMaterial(edgeDefaultMaterial);
    ...
    public boolean removeVertex(Vertex v, Group g){

        Map<Vertex,Edge> toRemove = new HashMap<>();

        if (!adjMapList.containsKey(v)) {
            return false;
        }else {
            //remove edges which end on vertex
            for (Vertex start : adjMapList.keySet()) {
                if (adjMapList.get(start) != null) {
                    for (Edge se : adjMapList.get(start)) {
                        if (se.getEndPoint().equals(v)) {
                            toRemove.put(start,se);
                        }
                    }
                }
            }
     ...
     * @param e
     * @param g - pass Group object if you want to delete it from the world
     * @return
     */
    public boolean removeEdge(Edge e, Group g){

        Map<Vertex, Edge> toRemove = new HashMap<>();

        bigloop:
        for(Vertex v: adjMapList.keySet()){
            if(adjMapList.get(v) != null){
                for(Edge se : adjMapList.get(v)){
                    if(se.equals(e)){
//                        adjMapList.get(v).remove(se);
                        toRemove.put(v,e);
                        break bigloop;
                    }
                }
    ...
    private void removeNode(Group g, Map<Vertex, Edge> toRemove, Vertex v) {
        if(g!=null){
            g.getChildren().remove(toRemove.get(v));
        }
    }

    public Vertex getRandomVertex(){
        Random rand = new Random();
        List<Vertex> vs = new ArrayList<>(getVertices());
        return vs.get(rand.nextInt(vs.size()));
    }

    public Edge getRandomEdge(){
        Random rand = new Random();
        return getEdges().get(rand.nextInt(getEdges().size()));
    }
...
//        Edge se = createEdge(edge.getStartPoint(),edge.getEndPoint());
    ...
    /**
     * Get edges which start with vertex of
     * @param of
     * @return
     */
    ...
    /**
     * Get edges which end on vertex of
     * @param of
     * @return
     */
